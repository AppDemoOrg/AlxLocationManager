package com.abt.location.manager;
...
import com.abt.location.Service.LocationService;
import com.abt.location.bean.Location;
import com.abt.location.task.LocateTask;
...
public class LocationManager implements GoogleApiClient.ConnectionCallbacks,
    ...
    public static LocationManager manager;//单例模式
    ...
    public static void onCreateGPS(final Application context) {
        ...
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                context.startService(new Intent(manager.context, LocationService.class));
                ...
                new LocateTask<Void, Void, String>() {//在子线程中获取附近的基站和wifi信息
                    ...
                    protected void onPostExecute(String json) {
                        ...
                        if (manager != null && context != null)
                            manager.sendJsonByPost(json, "https://www.googleapis.com/geolocation/v1/geolocate?key=" + GOOGLE_API_KEY);
                        else return;
    ...
    public static class LocationTask extends TimerTask {
        ...
        @Override
        public void run() {
            Log.i("AlexLocation", "location task 执行" + "manager是" + manager + "    destroy是" + LocationService.isDestory);
            if (manager == null || !LocationService.isDestory) return;//如果之前被destroy了，就重开一个
            manager.context.startService(new Intent(manager.context, LocationService.class));//使用安卓原生API获取地理位置
        }
    }
    ...
    class TowerAndWiFiTask extends LocateTask<Void, Void, String> {
        @Override
        protected String doInBackground(Void... params) {
            GeoLocationAPI geoLocationAPI = null;
            try {
    ...
    public static void recordLocation(Context context, double latitude, double longitude, float accuracy) {
        ...
        //中国境内换算成火星坐标系
        Location myLocation = gps84_To_Gcj02(latitude, longitude);
        ...
        Location myLocationStatic = Location.getInstance();
    ...
    public void sendJsonByPost(String json, String url) {
        ...
        HttpRequest.post(url, params, new BaseHttpRequestCallback<String>() {
            ...
            public void onFailure(int errorCode, String msg) {
                ...
                if (isDebugging)
                    Toast.makeText(context, "WIFI和基站定位失败，原因没有翻墙或者网络不通", Toast.LENGTH_LONG).show();
            }
        });
    }
    ...
    public static double getGPSDistance(double lat_a, double lng_a, double lat_b, double lng_b) {
        ...
        if (isDebugging && manager != null)
            Toast.makeText(manager.context, "位置移动了" + distance + "米", Toast.LENGTH_LONG).show();
        return distance;
    }
