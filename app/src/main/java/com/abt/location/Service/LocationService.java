package com.abt.location.Service;
...
public class LocationService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        while (!isDestory) {
            ...
            if ((locationProvider != null) && (PROVIDER_ARRAY.contains(locationProvider))) {//如果成功获取到了位置
                try {
                    if (!isWrongPosition(Location.getInstance().latitude, Location.getInstance().longitude)) {//当前获取到的经纬度
                        isDestory = true;
                    } else {
                        Thread.sleep(LocationManager.FAST_UPDATE_INTERVAL);//如果获取的是不正确的经纬度
                    }
    ...
    private void updateLocation() {
        ...
        if ((locationProvider != null) && (!locationProvider.equals("")) && (PROVIDER_ARRAY.contains(locationProvider))) {
            try {
                android.location.Location currentLocation = locationManager.getLastKnownLocation(locationProvider);
                Log.i("AlexLocation","通过旧版service取到GPS，经度"+currentLocation.getLongitude()+" 纬度"+currentLocation.getLatitude()+"  来源"+locationProvider);
                if(LocationManager.isDebugging) Toast.makeText(getApplicationContext(),"通过Android原生方法！！！"+"经度"+currentLocation.getLongitude()+" 纬度"+currentLocation.getLatitude(),Toast.LENGTH_LONG).show();
                ...
                if (currentLocation != null) {
                    ...
                    if(!isWrongPosition(newLatitude,newLongitude)) LocationManager.recordLocation(this,newLatitude,newLongitude,accuracy);
                    if(LocationManager.manager!=null)
                        LocationManager.manager.currentStatus = LocationManager.STATUS.NOT_TRACK;
